...
import M_tree.SplitFunction.SplitResult;
...
//	}
	...
	public List<DATA> search(DATA query, double distance){
		List<DATA> list = new ArrayList<>();
		root.search(query, distance, list, 0.0);
		return list;
	}
...
//	}
	...
	private abstract class Node extends IndexItem {
		...
		Map<DATA, IndexItem> children = new HashMap<>();
		Rootness       rootness;
		Leafness<DATA> leafness;
		...
		abstract void search(DATA query, double distance, List<DATA> list, double preDis);
	}
	...
	private class LeafNodeTrait extends NodeTrait implements Leafness<DATA> {
		...
		public void addChild(MTree.IndexItem child, double distance) {
			assert !thisNode.children.containsKey(child.data);
			thisNode.children.put((DATA) child.data, child);
			assert thisNode.children.containsKey(child.data);
		...
		public void _checkChildClass(MTree.IndexItem child) {
	...
	class NonLeafNodeTrait extends NodeTrait implements Leafness<DATA> {
		...
		public void doAddData(DATA data, double distance) {
			...
			for(IndexItem item : thisNode.children.values()) {
				...
				if(Constants.compare(childDistance, child.radius) > 0) {
					double radiusIncrease = childDistance - child.radius;
					if(Constants.compare(radiusIncrease, minRadiusIncreaseNeeded.metric) < 0) {
						minRadiusIncreaseNeeded = new CandidateChild(child, childDistance, radiusIncrease);
					}
				} else {
					if(Constants.compare(childDistance, nearestDistance.metric) < 0) {
		...
		public void addChild(MTree.IndexItem newChild_, double distance) {
			@SuppressWarnings("unchecked")
		...
		public void doRemoveData(DATA data, double distance) throws DataNotFound {
			for(IndexItem childItem : thisNode.children.values()) {
				...
				if(Constants.compare(Math.abs(distance - child.distanceToParent),
                        child.radius) <= 0) {
		...
		private Node balanceChildren(Node theChild) {
			...
			for(IndexItem child : thisNode.children.values()) {
				...
				if(anotherChild.children.size() > anotherChild.getMinCapacity()) {
					if(Constants.compare(distance, distanceNearestDonor) < 0) {
						distanceNearestDonor = distance;
						nearestDonor = anotherChild;
					}
				} else {
					if(Constants.compare(distance, distanceNearestMergeCandidate) < 0) {
			...
			if(nearestDonor == null) {
				...
				for(IndexItem grandchild : nearestDonor.children.values()) {
					...
					if(Constants.compare(distance, nearestGrandchildDistance) < 0) {
	...
	private class RootLeafNode extends Node {
		...
		@Override
		void search(DATA query, double distance, List<DATA> list, double preDis) {
			double parentDis = distanceFunction.calculate(data, query);
			for (IndexItem value : children.values()) {
				if ( Constants.compare(Math.abs(parentDis - value.distanceToParent), distance) <= 0){
					preDis = distanceFunction.calculate(value.data, query);
					if (Constants.compare(preDis, distance) <= 0)
						list.add(value.data);
				}
			}
		}
	...
	private class RootNode extends Node {
		...
		void search(DATA query, double distance, List<DATA> list, double preDis) {
			double parentDis = distanceFunction.calculate(data, query);
			for (IndexItem value : children.values()) {
				if ( Constants.compare(Math.abs(parentDis - value.distanceToParent), distance + value.radius) <= 0){
					preDis = distanceFunction.calculate(value.data, query);
					if (Constants.compare(preDis, distance + value.radius) <= 0)
						((Node)value).search(query, distance, list, preDis);
				}
			}
		}
	}
	...
	private class InternalNode extends Node {
		...
		void search(DATA query, double distance, List<DATA> list, double preDis) {
			for (IndexItem value : children.values()) {
				if ( Constants.compare(Math.abs(preDis - value.distanceToParent), distance + value.radius) <= 0){
					double valueDis = distanceFunction.calculate(value.data, query);
					if (Constants.compare(valueDis, distance + value.radius) <= 0)
						((Node)value).search(query, distance, list, valueDis);
				}
			}
		}
	}
	...
	private class LeafNode extends Node {
		...
		void search(DATA query, double distance, List<DATA> list, double preDis) {
			for (IndexItem value : children.values()) {
				if (Constants.compare(Math.abs(preDis - value.distanceToParent), distance) <= 0){
					double valueDis = distanceFunction.calculate(value.data, query);
					if (Constants.compare(valueDis, distance) <= 0)
						list.add(value.data);
				}
			}
		}
